...
resolve_npx() {
    ...
    # Try OpenCode's node
        ...
        NPX="$HOME/.opencode/bin/node"  # node can run npx as module
    fi
    if [ -z "$NPX" ] || [ ! -x "$NPX" ]; then
        # Fallback: try using node to run npx
...
# Helper function to run npx with fallback
run_npx() {
    if [[ "$NPX" == "node" ]]; then
        node "$HOME/.local/share/fnm/node-versions/node/v22.22.0/bin/node_modules/npm/bin/npx-cli.js" "$@"
    else
        "$NPX" "$@"
    fi
}
